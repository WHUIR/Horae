...
from recbole.model.rotary_embedding_torch import RotaryEmbedding
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.rotary_emb = RotaryEmbedding(dim=self.attention_head_size)
    ...
    def forward(self, input_tensor, attention_mask):
        mixed_query_layer = self.query(input_tensor) #(batch, seqlen, dim)
        mixed_key_layer = self.key(input_tensor)
        mixed_value_layer = self.value(input_tensor)

        # (batch, seqlen, heads, dim//heads) -> (batch, heads, seqlen, dim//heads)
        query_layer = self.transpose_for_scores(mixed_query_layer).permute(0, 2, 1, 3)
        key_layer = self.transpose_for_scores(mixed_key_layer).permute(0, 2, 3, 1)
        value_layer = self.transpose_for_scores(mixed_value_layer).permute(0, 2, 1, 3)

        # use RoPE (batch, seqlen, heads, dim//heads) ->
        query_layer = self.rotary_emb.rotate_queries_or_keys(query_layer)
        value_layer = self.rotary_emb.rotate_queries_or_keys(value_layer)
