...
class PretrainTrainer(object):
    def __init__(self, model, config, train_dataloader, valid_dataloader, test_dataloader):
        ...
        if self.config['load_model_path'] == "":  # or self.config['stage'] != 'train':
            ...
            self.saved_path = self.config['weights_path'] + "{}-{}-{}-{}/".format(local_time.tm_mday,local_time.tm_hour,local_time.tm_min,local_time.tm_sec)
            if not os.path.exists(self.saved_path):
                os.makedirs(self.saved_path)
        elif self.config['stage'] != 'pretrain':
            dir = self.config['load_model_path'].split('/')[-2]
            name = self.config['load_model_path'].split('/')[-1].split('.')[0]
            local_time = localtime()
            cnt = str(local_time.tm_mday) + str(local_time.tm_hour) + str(local_time.tm_min) + str(local_time.tm_sec)
            self.saved_path = self.config['weights_path'] + "{}-{}-{}-{}/".format(dir, name, self.config['dataset'],cnt)
            if not os.path.exists(self.saved_path):
                os.makedirs(self.saved_path)
        else:
            self.saved_path = self.config['weights_path'].join(self.config['load_model_path'].split('/')[:-1]) + '/'
    def set_optimizer(self, model, config):
        no_decay = ["bias", "LayerNorm.weight"]

        # parameters that will be optimized with learning rate config['lr2']
        downstream_parameters = []

        for _ in self.model.position_embedding.parameters():
            downstream_parameters.append(_)

        for _ in self.model.trm_encoder.parameters():
            downstream_parameters.append(_)

        for _ in self.model.trm.parameters():
            downstream_parameters.append(_)

        for _ in self.model.time_diff_embedding_t.parameters():
            downstream_parameters.append(_)

        for _ in self.model.time_diff_embedding_n.parameters():
            downstream_parameters.append(_)

        for _ in self.model.gated.parameters():
            downstream_parameters.append(_)

        # set of ids of downstream parameters
        downstream_ids = set(id(p) for p in downstream_parameters)

        optimizer_grouped_parameters = [
            # parameters that will be optimized with learning rate config['lr']
            {
                "params": [p for n, p in model.named_parameters() if
                           not any(nd in n for nd in no_decay) and id(p) not in downstream_ids],
                "weight_decay": config['regs'],
                "lr": config['lr'],
            },
            # parameters without weight decay that will be optimized with learning rate config['lr']
            {
                "params": [p for n, p in model.named_parameters() if
                           any(nd in n for nd in no_decay) and id(p) not in downstream_ids],
                "weight_decay": 0.0,
                "lr": config['lr'],
            },
            # downstream parameters that will be optimized with learning rate config['lr2']
            {
                "params": [p for n, p in model.named_parameters() if
                           any(nd in n for nd in no_decay) and id(p) in downstream_ids],
                "weight_decay": 0.0,
                "lr": config['lr2'],
            },
            {
                "params": [p for n, p in model.named_parameters() if
                           not any(nd in n for nd in no_decay) and id(p) in downstream_ids],
                "weight_decay": config['regs'],
                "lr": config['lr2'],
            }
        ]
    ...
    def early_stop(self, result, metric='recall', flag_step=10, epoch=0):
        if self.config['stage'] == 'pretrain':
